...
- `chores/` — single-file SPA (`chores/public/index.html`) mounted at `/chores`. State persisted via `GET/POST /api/chores` to a single JSONB row in `chores.state` (Postgres). Schema is bootstrapped on root-server startup; reuses `DATABASE_URL`. **Being replaced by `loadout/`** — stays live until the phase-2 cutover, then 301s.
- `loadout/` — Edward's daily quest tracker (successor to chores). Spec in `docs/loadout-spec.md`, module docs in `loadout/README.md`. Router factory `loadout/api.js` mounted at `/api/loadout`; two single-file SPAs (`public/index.html` for Edward at `/loadout`, `public/hq.html` for the parent at `/loadout/hq`, PIN-gated via `HQ_PIN`). Data in Postgres schema `loadout.*` on `DATABASE_URL`. All four spec phases shipped (migration, pack loop, quest check-ins, vault/rewards, history). `/chores` 301s to `/loadout/` automatically once `loadout.legacy` exists (i.e. the migration has run) — see the cutover runbook in `loadout/README.md`.
...
Loadout (`loadout/`):
```bash
npm test                                          # node:test — tz + scoring unit tests
TEST_DATABASE_URL=postgres://…/scratch npm test   # also runs the API integration test (TRUNCATES loadout.*)
DATABASE_URL=… npm run loadout:migrate -- --dry-run   # one-shot chores → loadout import (see loadout/README.md)
```
...
**Loadout rules live in `loadout/lib/`, not in the SPAs.** Day keys and timestamps come from `lib/tz.js` (Asia/Taipei) — never `toISOString().slice(0,10)`. Awards are computed by `lib/scoring.js` (pure, unit-tested) and written by `api.js` inside the same transaction as the day record (`store.withDay` holds `SELECT … FOR UPDATE`). Balances are `SUM()` over `loadout.ledger` — never store one. The pack-check award is paid on first submit only; `awarded`/`ledgerId` on the day record is the guard, so reopen + resubmit can't double-pay. Check-ins re-log by recomputing the award and paying only the delta over `checkin.paid` (must be ≥ 0 for Edward; parents adjust downward). Reward requests reserve coins but write nothing; the `spend` row is written on parent approval inside a row-locked transaction that re-checks the balance. Every coin award is split at pay time via `scoring.splitCoins` — `config.bank.share` (default 50%) to `bank`, the rest to spendable `coins`; spends never touch `bank`, and only `POST /ledger/adjust` (explicit buckets, note required) moves it. Parent-only routes go through `auth.requireHq`; with `HQ_PIN` unset the gate is open (dev) and the server warns. Static routing: `/loadout/hq*` → `hq.html`, any other `/loadout/*` → `index.html` (both use History routing with base `/loadout`).
